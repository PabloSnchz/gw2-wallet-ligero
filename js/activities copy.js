 ...
 *
 * Novedades v1.4.0:
 * - Fractales (mañana): prefetch en idle de /v2/achievements/categories/88?v=latest
 *   y píldora con tooltip “Mañana” (T4 + Recommended) en el bloque de Fractales.
 * - World Bosses dinámicos: nueva sección en Diarias con próximos 90 min
 *   (lista cronológica HH:MM local) y botón “Copiar” por entrada (chat code)
 *   usando un schedule embebido (sin dependencias).
 *
 * Novedades v1.3.6:
 * - Bloque “Fractales (hoy)” en Diarias:
 *   Usa /v2/achievements/categories/88 (hoy) y /v2/achievements para nombres.
...
(function (root) {
  ...
  // requestIdleCallback (polyfill suave)
  ...
  function renderFractalsDaily(){
    ...
    // Nota “Mañana” (si está disponible)
    const tm = state.fractals.tomorrow || { t4:[], rec:[] };
    let tomorrowNote = '';
    if ((tm.t4 && tm.t4.length) || (tm.rec && tm.rec.length)){
      const tip = [
        'Mañana —',
        tm.t4?.length ? ('T4: '+tm.t4.join(', ')) : 'T4: —',
        '·',
        tm.rec?.length ? ('Rec: '+tm.rec.join(', ')) : 'Rec: —'
      ].join(' ');
      tomorrowNote =
        '<div class="muted" style="margin-top:6px">'+
          '<span class="pill s-info" id="fractalsTomorrowPill" data-tip="'+esc(tip)+'" title="'+esc(tip)+'">Mañana (detalles)</span>'+
        '</div>';
    }
  ...
  // ====== World Bosses — schedule embebido (UTC) + cálculo próximos ======
  // Diseño: patrón de 2 horas con 4 bosses principales (baseline sin dependencias).
  // Futuros: se puede enchufar una fuente externa manteniendo este contrato.
  const WB_BASE_PATTERN_MIN = [
    // offsetMin dentro de un bloque de 120 min
  ...
  // Genera el schedule de un día (0..1439, UTC)
  function genWBScheduleDayUTC(){
    const out = [];
    for (let base=0; base<1440; base+=120){
      WB_BASE_PATTERN_MIN.forEach(p=>{
        out.push({ tMin: (base + p.offsetMin), name: p.name, chat: p.chat });
      });
    }
    // Ordenado por tiempo
    out.sort((a,b)=>a.tMin-b.tMin);
    return out;
  }
  const WB_DAY = genWBScheduleDayUTC();

  /** Devuelve próximos eventos dentro de windowMin minutos desde ahora (local),
   *  calculado con schedule UTC y devuelto con HH:MM local + tooltip HH:MM UTC.
   */
  function computeWorldBossesNextWindow(windowMin){
    const now = new Date();
    const nowUTCmin = (now.getUTCHours()*60 + now.getUTCMinutes()); // 0..1439
    const limUTC = nowUTCmin + windowMin;

    // Buscamos en hoy y mañana para wrap-around.
    const list = [];
    function pushEntry(tMinUTC, name, chat){
      const dayShift = Math.floor(tMinUTC / 1440);
      const m = tMinUTC % 1440;
      const hUTC = Math.floor(m/60), miUTC = m%60;
      ...
      const atLocalStr = pad2(atLocal.getHours())+':'+pad2(atLocal.getMinutes());
      const atUTCStr   = pad2(atUTC.getUTCHours())+':'+pad2(atUTC.getUTCMinutes())+' UTC';
      list.push({ atLocalStr, atUTCStr, name, chat, at: atLocal.getTime() });
    }

    // Hoy
    WB_DAY.forEach(e=>{
      if (e.tMin >= nowUTCmin && e.tMin <= limUTC) pushEntry(e.tMin, e.name, e.chat);
    });
    // Wrap a mañana (si límite cruza 24h)
    if (limUTC > 1439){
      const over = limUTC - 1440;
      WB_DAY.forEach(e=>{
        if (e.tMin <= over) pushEntry(e.tMin+1440, e.name, e.chat);
      });
    }

    // Siempre devolvemos en orden cronológico local
    list.sort((a,b)=>a.at-b.at);
    // Recorte de calidad: máximo 6 para evitar ruido visual
  ...
  function renderWorldBosses(){
    ...
    host.innerHTML = '<ul class="list">'+ next.map(e=>{
      const dis = e.chat ? '' : ' disabled';
      const tip = 'Hora del servidor: '+esc(e.atUTCStr);
      return (
        '<li>'+
          '<strong>'+esc(e.atLocalStr)+'</strong> — '+esc(e.name)+' '+
          '<button class="btn btn--xs btn--ghost" data-tip="'+tip+'" title="'+tip+'" data-wb-copy="'+esc(e.chat||'')+'"'+dis+'>Copiar</button>'+
        '</li>'
      );
    }).join('') + '</ul>';
    ...
    host.querySelectorAll('button[data-wb-copy]').forEach(btn=>{
      ...
      btn.addEventListener('click', function(){
        const code = this.getAttribute('data-wb-copy')||'';
...
// ====== Heredad “anti‑sábana” ======
      ...
      host.innerHTML = [
        ...
        '<div class="panel__body">',
          ...
          '<div id="actDaily" class="tab-panel" role="tabpanel" aria-labelledby="actTabDaily">',
            ...
            // === World Bosses (próximos 90 min) ===
            '<section class="card" id="wbSection">',
              '<h2>🌋 World Bosses (próximos 90 min)</h2>',
              '<div id="wbBody" class="muted">Calculando agenda…</div>',
            '</section>',
  ...
  async function activate(){
    ...
    // Fractales (mañana) — prefetch en idle + hydrate nota
    requestIdle(async ()=>{
      try {
        await fetchFractalsDaily('tomorrow');
        renderFractalsDaily();
      } catch(_) {}
    });

    // World Bosses — computar próximos 90 min (sin fetch)
  ...
  async function prefetch(ctx){
    try {
      ...
      // “Mañana” puede venir después en idle durante activate()
    } catch(_) {}
  }
  ...
  function wireGlobal(){
    ...
    document.addEventListener('gn:global-refresh', async function(){
      ...
      ensurePSNA();
      await fetchFractalsDaily('today');
      // “Mañana” si ya fue prefetcheado queda; si no, se mantiene call en idle
      renderFractalsDaily();
      // WB: recalcular ventana desde el “ahora”
      state.worldBosses.next = computeWorldBossesNextWindow(90);
      renderWorldBosses();
