 ...
 *
 * CAMBIOS v3.19.2:
 * - Agregado icono de Leivas (96978.png) a la izquierda del pill
 * - Cambio a color verde cuando Leivas llega a 5/5
 * - Barra de progreso cambia a verde al completar
 */
...
(function (root) {
  ...
  async function detectWeeklyKeyFromCharacters(token) {
    ...
    try {
      console.log(LOG, '[Key Detection] Buscando Thiefs recientes...');
      
      const listRes = await fetch(
        `https://api.guildwars2.com/v2/characters?access_token=${encodeURIComponent(token)}`
      );
      ...
      const characters = await Promise.all(
        characterNames.map(async (name) => {
          const detailRes = await fetch(
            `https://api.guildwars2.com/v2/characters/${encodeURIComponent(name)}?access_token=${encodeURIComponent(token)}`
          );
  ...
  // =======================================================================
  // 12. RENDERIZADO SEMANAL (CON ICONO Y COLOR VERDE)
  // =======================================================================
  function renderWeekly() {
    ...
    if (weeklyKeyStatus) {
      if (state.weekly.key) {
        weeklyKeyStatus.innerHTML = '<span class="badge badge--success" style="font-size: 0.9rem; padding: 6px 12px;">✓ Reclamada esta semana</span>';
      } else {
        weeklyKeyStatus.innerHTML = '<span class="badge badge--warning" style="font-size: 0.9rem; padding: 6px 12px;">⏳ Disponible</span>';
      }
    }
    ...
    // Leivas - actualizar texto y clase
    var pillLeivas = $('#pillLeivas');
    if (pillLeivas) {
      pillLeivas.textContent = 'Leivas: ' + state.weekly.stones + '/5';
      
      // Cambiar clase según estado
      if (state.weekly.stones >= 5) {
        pillLeivas.className = 'pill s-ok';
      } else {
        pillLeivas.className = 'pill s-pending';
      }
    }
    
    // Barra de Leivas - actualizar ancho y color
    ...
    if (barLeivas) {
      ...
      if (state.weekly.stones >= 5) {
        barLeivas.style.background = 'linear-gradient(90deg, #4CAF50, #8BC34A)';
      } else {
        barLeivas.style.background = 'linear-gradient(90deg, #ffd966, #ffaa33)';
      }
    }
  }
  ...
  // =======================================================================
  // 15. INICIALIZACIÓN DEL PANEL (CON ÍCONO EN EL TÍTULO Y NUEVA UI DE LLAVE)
  // =======================================================================
  ...
  function wireGlobal() {
    ...
    document.addEventListener('gn:tokenchange', async function(ev) {
      ...
      if (newToken) {
        var weeklyKeyStatus = $('#weeklyKeyStatus');
        if (weeklyKeyStatus) {
          weeklyKeyStatus.innerHTML = '<span class="badge badge--info">🔄 Verificando con nueva key...</span>';
        }
  ...
  var Activities = {
    initOnce: function() {
      ...
      wireGlobal();
      state.inited = true;
      console.info(LOG, 'ready v3.19.2 — Icono de Leivas y color verde al completar');
    },
    activate: activate,
